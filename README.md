...
The top-level `GAPIT()` pipeline supports GLM, MLM, CMLM, MLMM, gBLUP, and sBLUP
with an open labeled store. The direct `glm_gwas()`, `mlm_gwas()`,
`cmlm_gwas()`, `mlmm_gwas()`, `select_super_qtns()`, and `sblup()` APIs accept
the same chunk-readable input; `gblup()` consumes the already constructed
kinship matrix. Top-level stores supply their own taxa and marker metadata, must
contain finite pre-imputed values, omit `GM`, and currently use VanRaden
kinship. SUPER/sBLUP materializes only the largest requested pseudo-QTN
candidate pool and reuses its prefixes; MLMM materializes only bounded marker
batches and its small selected-cofactor design. FarmCPU and BLINK iterations,
plus cBLUP, still require an in-memory genotype matrix.
