...
The top-level `GAPIT()` pipeline supports GLM, MLM, CMLM, MLMM, FarmCPU, gBLUP,
and sBLUP with an open labeled store. The direct `glm_gwas()`, `mlm_gwas()`,
`cmlm_gwas()`, `mlmm_gwas()`, `farmcpu_gwas()`, `select_super_qtns()`, and
`sblup()` APIs accept the same chunk-readable input; `gblup()` consumes the
already constructed kinship matrix. Top-level stores supply their own taxa and
marker metadata, must contain finite pre-imputed values, omit `GM`, and
currently use VanRaden kinship. SUPER/sBLUP materializes only the largest
requested pseudo-QTN candidate pool and reuses its prefixes; MLMM and FarmCPU
materialize only bounded marker batches and their small selected-cofactor
designs. BLINK and cBLUP still require an in-memory genotype matrix.
