...
def test_top_level_cmlm_preprocessing_compression_and_final_table_match_gapit(
    r_bridge: RBridge,
    r_root: Path,
    fixed_gapit_inputs: tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame],
) -> None:
    """Connect raw labeled inputs to GAPIT's fixed-group CMLM output."""
    ...
    kinship_with_taxa = np.column_stack([
        np.arange(len(reference.taxa), dtype=np.float64),
        reference.kinship,
    ])
    ...
    r_group_kinship = r_bridge.float_array(r_bridge.component(r_compression, "kinship"))
    r_incidence = np.zeros((len(reference.taxa), n_groups), dtype=np.float64)
    r_incidence[np.arange(len(reference.taxa)), r_labels - 1] = 1.0

    py_group_kinship, py_incidence = compress_kinship(reference.kinship, n_groups)
    py_labels = np.argmax(py_incidence, axis=1) + 1
    nt.assert_array_equal(
        py_labels[:, None] == py_labels[None, :],
        r_labels[:, None] == r_labels[None, :],
    ...
    r_result = r_mlm(
        ys=r_bridge.matrix(reference.phenotype_values[np.newaxis, :]),
        xs=r_bridge.matrix(reference.genotype_values),
        K=r_bridge.matrix(r_group_kinship),
        Z=r_bridge.matrix(r_incidence),
        X0=r_bridge.matrix(reference.design),
        CVI=r_bridge.matrix(covariates_with_taxa),
        file_from=1,
        file_to=1,
        file_fragment=reference.genotype_values.shape[1],
        fullGD=True,
        SNP_P3D=True,
        Timmer=r_null,
        Memory=r_null,
